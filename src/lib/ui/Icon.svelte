...
import(/* @vite-ignore */`./images/${name}.svg?raw`).then( (module) => {
  if(!module){
    iconSvg = ''
  }else{
...
<style>
:global(.theme-dark svg[colornotdefined=true]){fill:#fff !important}
:global(.theme-light svg[colornotdefined=true]){fill:#222 !important}
